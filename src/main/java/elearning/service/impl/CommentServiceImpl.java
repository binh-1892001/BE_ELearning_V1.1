...
public class CommentServiceImpl implements CommentService {
	...
	public void deleteComment(Long id) throws CustomException {
		Comment comment = commentRepository.findById(id).orElseThrow(() -> new CustomException("comment not found"));
		commentRepository.deleteAllByCommentId(id);
	...
	@Override
	public List<CommentDto> getAllCommentByLesson(Long idLesson) {
		List<CommentDto> comments = commentRepository.findAllByLessonId(idLesson).stream().filter(comment -> comment.getComment()==null).map(comment -> new CommentDto(comment)).toList();
		comments = comments.stream().sorted(Comparator.comparingLong(CommentDto::getId).reversed()).toList();
		return comments;
	}
}
